          ...
          do (let ((closest-distance 999999)
               ...
               (loop for target in (food *state*)
                     ...
                     do (if (< this-distance closest-distance)
                          ...
                          nil))
               (setf direction (get-random-elt (if closest-target
                                                 (get-directions a-row a-col (car closest-target) (cadr closest-target))
                                                 '(nil nil nil nil))))
               ;(append destinations (new-location a-row a-col direction))
               (issue-order a-row a-col direction)))))
